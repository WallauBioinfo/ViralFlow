...
parser = argparse.ArgumentParser(
    description="This scripts returns putative minor variants, based on nucleotide diversity of reads mapped against a reference genome"
)
parser.add_argument(
    "-in", "--bam_readcount", help="bam-readcount output file", required=True
)
parser.add_argument(
    "-lm",
    "--per_limit",
    help="Percentage limit of Depth Base / Total Base to consider as putative minor variant, default = 0.05, which represents 5 percent",
    type=float,
    default=0.05,
)
parser.add_argument(
    "-dp",
    "--depth_number",
    help="Minimum depth value to consider as minor variant, default = 100",
    type=int,
    default=100,
)
parser.add_argument(
    "-al",
    "--alignment_file",
    help="Alignment file against wuhan reference genome, with keeplength parameter",
    required=True,
)
parser.add_argument(
    "-gf", "--gff_file", help="Gff3 file of reference genome", required=True
)
...
def get_nr_regions(gff_file):
    """
    This function format a gff file, to keep non-redundant ranges, based on UTR and gene names
    """
    gff3_head = [
        "seqid",
        "source",
    ...
    if df["source"].str.contains("RefSeq").any():
        ...
        df = df[
            df.type.str.contains("UTR")
            | df.type.str.contains("mature_protein_region_of_CDS")
        ]
...
def format_bam(bam_rc_file, gff_df):
    """
    ...
    with open(bam_rc_file, "r") as bc_file, open(
        ...
        for line in bam_rc_file_read:
            ...
            if (
                len(line) > 11
            ):  # bamreadcount output can have 11 or more columns, depending of number of putative indels
            ...
            if "pos" in del_dict and int(line[1]) in del_dict["pos"]:
                ...
                out_list.append(
                    [
                        ...
                        var_pos,
                        region,
                        int(var_depth) + int(del_dict["del_depth"][item_index]),
                        A_depth,
                        A_plus,
                        A_minus,
                        C_depth,
                        ...
                        T_plus,
                        T_minus,
                        del_dict["del_depth"][item_index],
                        del_dict["del_depth_plus"][item_index],
                        del_dict["del_depth_minus"][item_index],
...
def generate_intrahost(bam_rc_formated):
    """
    This identify genomic positions with a intrahost signal, based on a set of conditions, such as: minimum depth, presence of intrahost in both read senses and minimum total depth
    """
    with open(bam_rc_formated, "r") as bam_readcount_formated:
        df = pd.read_csv(bam_readcount_formated, sep="\t", header=0)
        conditions = [
            # A/T
            (df["A_DEPTH"] >= depth_value)
            & (df["T_DEPTH"] > depth_value)
            & (df["A_DEPTH"] / df["DEPTH"] >= per_limit)
            & (df["T_DEPTH"] / df["DEPTH"] >= per_limit)
            & (df["C_DEPTH"] / df["DEPTH"] < per_limit)
            & (df["G_DEPTH"] / df["DEPTH"] < per_limit)
            & (
                (df["A_PLUS"] / df["A_MINUS"] > 0.05)
                & (df["INDEL_DEPTH"] / df["DEPTH"] < per_limit)
                & (df["A_MINUS"] / df["A_PLUS"] > 0.05)
            )
            & (
                (df["T_PLUS"] / df["T_MINUS"] > 0.05)
                & (df["T_MINUS"] / df["T_PLUS"] > 0.05)
            ),
            # A/C
            (df["A_DEPTH"] >= depth_value)
            & (df["C_DEPTH"] > depth_value)
            & (df["A_DEPTH"] / df["DEPTH"] >= per_limit)
            & (df["C_DEPTH"] / df["DEPTH"] >= per_limit)
            & (df["T_DEPTH"] / df["DEPTH"] < per_limit)
            & (df["G_DEPTH"] / df["DEPTH"] < per_limit)
            ...
            & (
                (df["C_PLUS"] / df["C_MINUS"] > 0.05)
                & (df["C_MINUS"] / df["C_PLUS"] > 0.05)
            ),
            # A/G
            (df["A_DEPTH"] >= depth_value)
            & (df["G_DEPTH"] > depth_value)
            & (df["A_DEPTH"] / df["DEPTH"] >= per_limit)
            & (df["G_DEPTH"] / df["DEPTH"] >= per_limit)
            & (df["C_DEPTH"] / df["DEPTH"] < per_limit)
            & (df["T_DEPTH"] / df["DEPTH"] < per_limit)
            ...
            & (
                (df["G_PLUS"] / df["G_MINUS"] > 0.05)
                & (df["G_MINUS"] / df["G_PLUS"] > 0.05)
            ),
            # A/INDEL
            (df["A_DEPTH"] >= depth_value)
            & (df["INDEL_DEPTH"] > depth_value)
            & (df["A_DEPTH"] / df["DEPTH"] >= per_limit)
            & (df["INDEL_DEPTH"] / df["DEPTH"] >= per_limit)
            & (
                (df["A_PLUS"] / df["A_MINUS"] > 0.05)
            ...
            & (
                (df["INDEL_PLUS"] / df["INDEL_MINUS"] > 0.05)
                & (df["INDEL_MINUS"] / df["INDEL_PLUS"] > 0.05)
            ),
            # T/C
            (df["T_DEPTH"] >= depth_value)
            & (df["C_DEPTH"] > depth_value)
...
def format_intrahost(intrahos_formated):
    """
    This function format the final intrahost files.
    """
    output_tsv = open(bam_rc_file + ".intrahost.tsv", "w")
    output_tsv_writer = csv.writer(output_tsv, delimiter="\t")
    output_tsv_writer.writerow(
        [
            "GENOME",
            "POS",
            "REGION",
    ...
    with open(intrahos_formated, "r") as minor_tsv:
        major = ""
        minor = ""
        major_depth = ""
        minor_depth = ""
        ...
        for line in minor_tsv_reader:
            ...
            if "GENOME" not in line[0]:
                try:  # For intrahost loci with 3 or more alleles, don't check frequency
                    frequency = "{:.2f}".format(
                        int(minor_depth) / int(line[3].rstrip("\n"))
                    )
                except Exception:
                    frequency = "CURATION"
                out_list_lists.append(
                    [
                        line[0],
                        line[1],
                        line[2],
...
def generate_minor_consensus(alignment_file):
    """
    This function generates a consensus genomic version with the minor iSNVs.
    """
    output_handle_min = open(alignment_file + ".minor.fa", "w")
    alignment = AlignIO.read(open(alignment_file), "fasta")
    ref_sequence = alignment[0].id
...
if __name__ == "__main__":
    ...
    generate_minor_consensus(alignment_file)
    remove_tmp()
